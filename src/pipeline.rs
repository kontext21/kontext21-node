...
#[napi(catch_unwind)]
pub async fn capture_and_process_screen(js_capture_config: JsScreenCaptureConfig, js_processor_config: JsProcessorConfig) -> napi::Result<JsImageDataCollection> {
    ...
    let result: Vec<ImageData> = run_live_screen_capture_ocr(&capture_config, &processor_config).await.into();
